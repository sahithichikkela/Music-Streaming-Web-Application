...
const MusicPlayer = () => {
    ...
    return(
        <div className='w-full full flex items-center gap-3 '>
            <div
                className={`w-full full items-center gap-3 p-4 flex relative`}>
                <img
                    src={allSongs[songIndex]?.imageURL}
                    className="w-40 h-20 object-cover rounded-md"
                    alt=""
                />
                {/* Content inside Music Player*/}
              ...
              ({allSongs[songIndex]?.category})
                    ...
                    >
                        {/* Playlist Button */}
                        <RiPlayListFill className="text-textColor hover:text-headingColor text-3xl cursor-pointer absolute" />
                    </motion.i>

                    <motion.i
                        whileTap={{ scale: 0.8 }}
                        onClick={() => setIsLyric(!isLyric)}
                    >
                        <IoIosMusicalNotes className="text-textColor hover:text-headingColor text-3xl cursor-pointer ml-11"/>
                    </motion.i>
...
export const PlayistCard = () => {
    ...
    return(
        <div className="absolute left-4 bottom-24 gap-2 py-2 w-350
         max-w-[350px] h-510 max-h-[510px]
        flex flex-col overflow-y-scroll scrollbar-thin rounded-md
            ...
            {allSongs.length > 0 ? (
                allSongs.map((music, index) => (
                    ...
                    >
                        ...
                        <div className="flex items-start flex-col">
                            <p className="text-lg text-headingColor font-bold">
                                {`${
